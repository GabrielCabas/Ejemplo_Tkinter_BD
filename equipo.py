...
class equipo:#Clase de equipo, puede llamar a las clases de insertar y modificar
    def __init__(self, db):
        self.db = db
        self.data = []
        self.root = tk.Toplevel()#Toplevel es una ventana que está un nivel arriba que la principal
    ...
    def config_buttons_jugador(self):#Botones de insertar, modificar y eliminar
        tk.Button(self.root, text="Insertar equipo", command = self.insertar_equipo).place(x = 0, y = 350, width = 200, height = 50)
        tk.Button(self.root, text="Modificar equipo", command = self.modificar_equipo).place(x = 200, y = 350, width = 200, height = 50)
        tk.Button(self.root, text="Eliminar equipo", command = self.eliminar_equipo).place(x = 400, y = 350, width = 200, height = 50)
    def llenar_treeview_equipo(self):#Se llena el treeview de datos. 
        sql = "select * from equipo"
        data = self.db.run_select(sql)#Ejecuta el select
        if(data != self.data):#Si la data es distina a la que hay actualmente...
            self.treeview.delete(*self.treeview.get_children())#Elimina todos los rows del treeview
            for i in data:
                self.treeview.insert("", "end", text = i[0], values = (i[1], i[2]), iid = i[0])#Inserta los datos
...
class insertar_equipo:#Clase para insertar data
    ...
    def config_button(self):#Se configura el boton
        tk.Button(self.insert_datos, text = "Aceptar", command = self.insertar).place(x=0, y =70, width = 200, height = 20)

    def insertar(self): #Insercion en la base de datos. 
        sql = """insert equipo (nom_equipo, puntos) values (%(nom_equipo)s, %(puntos)s)"""
        self.db.run_sql(sql, {"nom_equipo": self.entry_nombre.get(), "puntos": int(self.entry_puntos.get())})
...
class modificar_equipo:#Clase para modificar
    ...
    def config_button(self): #Botón aceptar, llama a la función modificar cuando es clickeado. 
        tk.Button(self.insert_datos, text = "Aceptar", command = self.modificar).place(x=0, y =70, width = 200, height = 20)

    def modificar(self): #Insercion en la base de datos. 
        sql = """update equipo set nom_equipo = %(nom_equipo)s, puntos = %(puntos)s where id_equipo = %(id_equipo)s"""
        self.db.run_sql(sql, {"nom_equipo": self.entry_nombre.get(), "puntos": int(self.entry_puntos.get()), "id_equipo": int(self.row_data[0])})
        self.insert_datos.destroy()
        self.padre.llenar_treeview_equipo()